...
# across()
# Suppose we need to apply the same transformation to several 
# variables. The function across facilitates the operation. 
# For example if want to log transform both population and 
# total murders we can use:
mutate(murders, across(c(population, total), log10))


# We can also use helper functions with across() for 
# numeric variables
